...
void GetFileNames(string path, vector<string>& filenames, vector<string>& filenames_file)
{
    DIR *pDir;
    struct dirent* ptr;
...
int main(int argc, char* argv[])
{
  ...
  std::string _input_path = input_path;

  if (endsWith(_input_path, "/"))
  {
    _input_path.pop_back();
  }
  GetFileNames(_input_path, dir_path, dir_name);
  for(int ii = 0; ii <dir_path.size(); ii++)
  {
      vector<string> img_path;
      vector<string> img_name;
      GetFileNames(dir_path[ii], img_path, img_name);
      for(int j = 0; j <img_path.size(); j++)
      {
        cv::Mat orig_img = imread(img_path[j].c_str(), cv::IMREAD_COLOR);
        if (!orig_img.data) {
        ...
        for (uint32_t i = 0; i < io_num.n_output; i++) {
          ...
          if (MaxClass[j] == std::stoi(dir_name[ii]))
          {
            true_count++;
          }
          std::cout << "pred = " << MaxClass[j]<< ",  label = " << std::stoi(dir_name[ii])  << std::endl;
          // printf("---- Top%d ----\n", top_count);
  ...
  std::cout << "sum = " << sum << ", true_count = " << true_count << ", false_count = " << sum - true_count << std::endl;
  std::cout << "Accuracy = " << float(true_count) / float(sum) << std::endl;
